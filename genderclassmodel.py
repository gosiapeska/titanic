...
for row in test_file_obect:
    ...
    if row[2] == 'female':
        row.insert(0,int(survival_table[0,int(row[0]),bin_fare])) #Insert the prediciton
                                                        #at the start of the row
        open_file_object.writerow(row) #Write the row to the file
    else:
        row.insert(0,int(survival_table[1,int(row[0])-1,bin_fare])) #Insert the prediciton
                                                        #at the start of the row
